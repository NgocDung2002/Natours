...
exports.isLoggedIn = async (req, res, next) => {
  try {
    if (req.cookies.jwt) {
      ...
      // 3) Check if user changed their password after the token was issued
      if (currentUser.changedPasswordAfter(decoded.iat)) {
        return next();
      }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('Token has not expired or invalid', 400));
